    ...
    <>
      <div className="grid h-[280px] grid-cols-4 grid-rows-2 gap-2 overflow-hidden rounded-lg sm:h-[420px]">
        <button onClick={() => setOpen(0)} className="col-span-2 row-span-2 cursor-pointer">
          <CoverArt seed={images[0].seed} photoUrl={images[0].url} className="h-full w-full transition-opacity hover:opacity-90" />
        </button>
        {images.slice(1, 5).map((img, i) => (
          <button key={img.seed} onClick={() => setOpen(i + 1)} className="relative cursor-pointer">
            <CoverArt seed={img.seed} photoUrl={img.url} className="h-full w-full transition-opacity hover:opacity-90" />
            {i === 3 && images.length > 5 && (
        ...
        {open !== null && (
          ...
          >
            ...
            <div className="relative flex flex-1 items-center justify-center px-6 pb-6">
              <button
                onClick={() => setOpen((o) => (o === null ? 0 : (o - 1 + images.length) % images.length))}
              ...
              >
                <CoverArt seed={images[open].seed} photoUrl={images[open].url} className="h-full w-full" rounded="rounded-lg" />
              </motion.div>
              <button
                onClick={() => setOpen((o) => (o === null ? 0 : (o + 1) % images.length))}
