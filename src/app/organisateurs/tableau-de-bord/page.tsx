...
import { hasActiveBoost, getBoostHistoryForOrganizer } from "@/lib/boosts";
...
export default async function OrganizerDashboardPage() {
  ...
  const [rawStages, boostHistory] = await Promise.all([
    getOrganizerStageRows(user.id),
    getBoostHistoryForOrganizer(user.id),
  ]);
  const stages = await Promise.all(
    rawStages.map(async (s) => ({ ...s, boosted: await hasActiveBoost(s.id) }))
  );
  ...
  return (
    <div className="container-page py-10">
      ...
      <div className="mt-8">
        <OrganizerDashboard stages={stages} revenueByMonth={[]} boostHistory={boostHistory} />
      </div>
    </div>
  );
}
